...
type AppAction =
  ...
  | { type: 'START_ANALYSIS' }
  | { type: 'COMPLETE_ANALYSIS'; payload: { resumeSkills: string[], jobSkills: string[], missingSkills: string[] } }
...
const appReducer = (state: AppState, action: AppAction): AppState => {
  switch (action.type) {
    ...
    case 'COMPLETE_ANALYSIS': {
      ...
      const { resumeSkills, jobSkills, missingSkills } = action.payload;
      
      // Create learning path
      let learningPath = createLearningPath(missingSkills);
      
      // Add mock videos to each skill
      learningPath = learningPath.map(phase => ({
        ...phase,
        skills: phase.skills.map(skill => ({
          ...skill,
          videos: getMockVideosForSkill(skill.name)
        }))
      }));
      
      // Create quizzes for each phase
    ...
    case 'COMPLETE_VIDEO': {
      ...
      // Check if all videos for this skill are completed
      ...
      const updatedPhases = newState.learningPath;
      for (const phase of updatedPhases) {
        for (const skill of phase.skills) {
          if (skill.id === skillId && skill.videos.every(video => video.isCompleted)) {
            return {
              ...newState,
              learningPath: newState.learningPath.map(p => ({
                ...p,
                skills: p.skills.map(s => {
                  if (s.id !== skillId) return s;
                  return { ...s, isCompleted: true };
                }),
                progress: calculatePhaseProgress(p.skills.map(s => 
                  s.id === skillId ? { ...s, isCompleted: true } : s
                ))
              })),
              userProgress: {
                ...newState.userProgress,
                completedSkills: [...newState.userProgress.completedSkills, skillId]
              }
            };
          }
        }
    ...
    case 'COMPLETE_SKILL': {
      ...
      // Check if any phase is completed
...
// Helper to calculate phase progress based on completed skills
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchVideosForSkills = async () => {
      if (state.learningPath.length > 0 && state.learningPath.every(phase => phase.skills.every(skill => skill.videos.length === 0))) {
        for (const phase of state.learningPath) {
          for (const skill of phase.skills) {
            const videos = await fetchEducationalVideos(skill.name);
            dispatch({ type: 'SET_SKILL_VIDEOS', payload: { skillId: skill.id, videos } });
          }
        }
      }
    };

    fetchVideosForSkills();
  }, [state.learningPath]);
  ...
  const analyzeData = async () => {
    ...
    try {
      ...
      // Dispatch results after a slight delay for better UX
      setTimeout(() => {
        dispatch({ 
